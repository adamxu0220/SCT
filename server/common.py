...
COMMANDS = {
			...
			'put': ('put <local file path> <remote file name>', 'Uploads a file with a given filename'),
                        'upload': ('upload <local file path> <remote file name> <cmdid>', 'Uploads a file with a given filename'),
			'hb': ('HB owner, hostname macaddr duttype dutip swversion, status', 'Heartbeat with owner hostname macaddr duttype dutip swversion, status'),
                        'case_finished': ('case_finished', 'case finish message'),
...
def display_message(logger, message):
	...
	logger.trace('%s %s' % (timestamp(), message))
...
def get_case_name(logger, id):
    if str(id) == "1":
        return os.path.join("BG2CDP", "Chromecast_HBOGO.tar.gz")
    elif str(id) == "2":
        return os.path.join("BG2CDP", "Chromecast_PlayMovie.tar.gz")
    elif str(id) == "3":
        return os.path.join("BG2CDP", "Chromecast_Netflix.tar.gz")
    elif str(id) == "4":
        return os.path.join("BG2CDP", "Chromecast_MLB_Nexus7.tar.gz")
    else:
        logger.error("invalid test type:%s"%str(res[1]))
        return None
